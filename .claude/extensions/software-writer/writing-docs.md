...
- `docs.pointer_file` = `AGENTS.md`. It is the only committed pointer convention: each one opens with `@README.md` and then carries LLM navigation for its directory. `CLAUDE.md` is untracked here — the companion-file rule does not apply, and the pointer branch never produces or edits a `CLAUDE.md`.
