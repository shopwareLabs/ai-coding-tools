...
| storefront JS | tests | `jest_run` |
| storefront JS | build | `webpack_build` |

Call tools on the same server sequentially; tools on different servers may run in parallel. Pass `scope` through when given. The verbose tool output stays in your context — never return it raw.
...
- `autofixable`: list a check finding here only when its fixer was **not** run this pass — `ecs_check`→`ecs_fix`, `rector_check`→`rector_fix`, `eslint_check`→`eslint_fix`, `stylelint_check`→`stylelint_fix`, `prettier_check`→`prettier_fix`. PHPStan, PHPUnit, `tsc_check`, and Jest findings are not mechanically fixable — keep them under results.
