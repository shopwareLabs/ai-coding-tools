...
- **An existence probe runs after validation.** The tool checks that the environment reaches the path the worktree maps to, through the same command wrapper the call itself uses, so a directory the environment cannot see — an unsynced Vagrant folder, say — refuses the call and names the mapped path. It runs under the four container environments; under `native` the mapped path is the host path validation already checked. A passing result is cached per environment and root for the life of the server process, and a failure is not cached.
