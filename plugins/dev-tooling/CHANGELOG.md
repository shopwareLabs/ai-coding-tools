...
### Added
- **Worktree support: a `project_root` parameter on every tool except `cwd`, across all three servers.** A call may target a linked git worktree of the launch root instead of the tree the server started in. The root is validated against git's own worktree registry before the call runs, and the call is refused when the dependencies the tool needs (`vendor/autoload.php`, `node_modules`) are missing under the resolved directory. Effectively needs git 2.48 or newer: resolution needs 2.31 (`git rev-parse --path-format`), and the mandatory relative-linkage rule plus its repair command are 2.48 features — on 2.31–2.47 every stock worktree is refused with a remedy that git cannot run. Below either floor the refusal names the version it needs.
- **Worktree targeting is accepted under all five environments.** A worktree target runs under `native`, `docker`, `docker-compose`, `vagrant` and `ddev` alike; no environment excludes it. Under `docker`, `docker-compose`, `vagrant` and `ddev` the worktree has to sit inside the launch project root, the only tree the container or VM mounts, and its environment-side path is the environment's configured workdir plus its position below that root — under `docker-compose`, a configured `docker-compose.workdir` takes the place of the match, and without one the destination of the longest matching bind mount does. A `ddev` call against a worktree always names the working directory with `ddev exec -d`, where an unscoped launch-root call keeps its existing argv.
- **A worktree whose `.git` file carries an absolute `gitdir` pointer is refused, under `native` included.** An absolute pointer names a host path that does not exist inside a container, and `git worktree add` writes one unless the repository sets `worktree.useRelativePaths` — so on a fresh checkout the refusal is the ordinary case, and it carries the remedy: `git -c worktree.useRelativePaths=true worktree repair <worktree-path>`, which needs git 2.48 or newer.
- **An existence probe runs after validation.** The tool checks that the environment reaches the path the worktree maps to, through the same command wrapper the call itself uses, so a directory the environment cannot see — an unsynced Vagrant folder, say — refuses the call and names the mapped path. It runs under the four container environments; under `native` the mapped path is the host path validation already checked. A passing result is cached per environment, root and mapped path for the life of the server process, and a failure is not cached.
- **A worktree's own `.mcp-php-tooling.json` or `.mcp-js-tooling.json` is honored per call.** When the worktree carries one, its scalars and the `environment` field it declares are derived for that call, leaving later calls against the launch root on the environment the server started with.
- **`set_project_root` and `cwd`, on all three servers.** `set_project_root` validates and sticks a project root for every later call on that server process until cleared; `cwd` reports the effective root, its source (`launch` or `sticky`), whether it resolves, the resolved working directory, the environment, and the configuration in use. Neither is subject to worktree resolution itself, so both keep working when a sticky root names a directory that no longer exists.
- **A `PostToolUse` hook on `EnterWorktree`/`ExitWorktree`.** `hooks/scripts/worktree-directives.sh` reminds the session to call `set_project_root` on all three servers after either event, since each server is a separate process holding its own sticky root. On enter it also reads the worktree's `.git` file and, when the pointer is absolute — which `EnterWorktree` writes unless the repository sets `worktree.useRelativePaths` — names the repair command up front, and it warns that a native worktree branches from the default remote branch unless the Claude Code setting `worktree.baseRef` is `head`.
- **`worktree_prepare`, on all three servers.** A fresh worktree holds tracked files only, so `vendor/` and `node_modules` are missing and the composer- and npm-backed tools refuse until they exist. The tool installs the server's own toolchain dependencies in the target root through the configured environment — `composer install --no-interaction` on `php-tooling`, `npm ci` in the package directory on the JS servers — which under a container runs the install inside the container at the mapped path. It skips the dependency check the other tools run, and the dependency refusals now name it as the remedy. A successful install returns a short completion summary instead of the per-package installer output; a failed one returns the full output. Symlinking `vendor/` from the launch tree is not an alternative and the tool description says why: composer's autoloaders resolve through the symlink and load the launch tree's classes.
...
- **`config.sh`'s cleanup is now the public `config_cleanup`**, renamed from `_config_cleanup` because six callers outside the module reached for the private name: the `EXIT` trap in each of the three servers, and three test teardowns. The `run_mcp_server` call and its rationale move into `shared/server_run.sh`, which each server sources; the `EXIT` trap stays in each `server.sh`, since moving it would run the server loop before the libs are sourced.
- **`get_environment_info` is removed from the shared environment module.** It reported the detected environment and had no callers.

### Fixed
- **Six tools accepted a `scope` and ignored it.** `lint_all`, `lint_twig`, `unit_setup`, `vite_build`, `webpack_build` and `phpunit_coverage_gaps` declared a `scope` parameter in their server's `tools.json` and never called `resolve_scope`, so a scoped call ran against the project root and said nothing about it. `phpunit_coverage_gaps` was the most visible of the six: a scoped project's Clover report sits under the scope's own cwd, and the tool read the project root. Each now resolves the scope its siblings already did, between `worktree_enter` and the dependency assertion.
- **Three unrelated refusals shared one phrase.** A configuration file that does not parse, tool-call arguments that do not parse, and `set_project_root` arguments that do not parse all reported "not a JSON object", so neither a reader nor a test could tell which one fired. Each reason now carries wording the other two do not.
...
- **An unrecognized environment name in a worktree's own configuration ran the command on the host.** The environment value selects which guards apply, and a name outside the known set — a `"docker "` typo, `"podman"` — fell through to the permissive native path, skipping containment, the charset rule and the probe while the launch wrappers stayed container-keyed. A worktree target now refuses unrecognized names; the known set lives in one `_env_is_container_environment` predicate in the shared environment module, read by containment, charset and probe alike, so an environment added there is picked up by all three guards in the same edit.
- **The probe cache reused a stale pass after a worktree's own configuration changed its workdir.** The cache was keyed on (environment, root) while the mapped path derives from the per-call re-read configuration; the mapped path is now part of the key, so an edited workdir probes again.
- **A scoped `worktree_prepare` on `php-tooling` installed where the dependency gate does not look.** The install ran in the scope's cwd while the gate checks `vendor/autoload.php` at the effective root, so the refusal's named remedy could never clear the refusal. The PHP tool no longer resolves a scope and its schema drops the parameter; the JS tools keep theirs, whose install and gate both derive the same package directory through `get_js_workdir`.
- **The `EnterWorktree` directive lost its repair hint on a `.git` file without a trailing newline**, because the read guard cleared a line `read` had populated; and both the directive and the server refusal embedded the repair path unquoted, so a root carrying a space split into wrong arguments when copy-pasted. The repair command now carries the path quoted in both messages.
- **`jest`'s `install_if_missing` tested an environment-side path on the host.** Under a container the composed `node_modules` path never exists on the host (and under `docker-compose` it is built from a literal sentinel), so every scoped `jest_run` paid a full `npm ci` first. The check now measures the host-side package directory, the same binding the dependency gate uses.
- **`dev-tooling-runner` could be steered into dependency installs.** The runner's server wildcard granted the new `worktree_prepare`, a `vendor/`/`node_modules`-rewriting setup command outside its documented mutation boundary — and the dependency refusals name that tool as the remedy. It is now subtracted via `disallowedTools`; the runner reports such refusals instead of acting on them.
...
### BREAKING CHANGE
- **A call carrying a parameter the tool does not declare is now refused.** Every tool's `inputSchema` sets `additionalProperties: false`, so a misspelled or unknown parameter returns `Unknown parameter(s): <name>` instead of being accepted and ignored. Remove the parameter, or correct its spelling to the declared name. Every tool function's argument reads were checked against its declared properties first, so no tool refuses a parameter its own code still reads.
