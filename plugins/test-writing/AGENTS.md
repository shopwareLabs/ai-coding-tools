...
test-writing:phpunit-test-team-reviewing (Skill, inline)
    ...
    ├── Phase 2: Assemble run input on disk — one rule package per test type present (build_rule_package per group/test_type) + placement package when integration files present; jq --rawfile splices catalogs by path into args.json; manifest fixed
    ├── Phase 3: Build flat run-script (workflow/build-run-script.sh splices the manifest into a top-level copy of team-review.workflow.mjs) → launch via Workflow tool scriptPath (no args)
...
Sole Workflow-based team reviewer for **unit, integration, and migration** PHPUnit tests in one run over a mixed manifest. Resolves input to a file list, classifies each path (`test_type`), fans out per-file extraction to parallel general-purpose/haiku subagents (measurements, source resolution, fingerprint, digest), assembles the run input on disk (one rule package per test type present + a placement package when integration files are present), splices it into a flat top-level copy of the committed workflow script (`skills/phpunit-test-team-reviewing/workflow/team-review.workflow.mjs`) via `workflow/build-run-script.sh`, launches that copy through the Workflow tool's `scriptPath`, and renders the result. Strictly read-only — it never mutates the tests under review. The committed script owns the orchestration; the `references/` provide the run's execution-phase contracts and the adaptation guide.
...
| Change adversary count | `team-reviewing/workflow/team-review.workflow.mjs` (`LENSES` array — adversary count = lens count, no formula) + `team-reviewing/references/reviewer-allocation.md` (adaptation guide) |
...
| Change run-script delivery / launch | `team-reviewing/workflow/build-run-script.sh` (manifest splice + marker guard + jq gate) + `team-reviewing/SKILL.md` Phase 2–3 (on-disk assembly + scriptPath launch) |
