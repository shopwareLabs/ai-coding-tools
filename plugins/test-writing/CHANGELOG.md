...
### Changed
- **Team-review migration capstone — commit the parameterized team-review Workflow script; remove the stale `team-review-workflow.mjs`.** Tier-0 capstone (SPEC 4 / R0 + C1) from the Jun 21–24 multi-run analysis. This is the step the prior four CHANGELOG entries deferred to ("a later migration step crystallizes the stabilized design into a committed script"): it lands last because committing the script is a change of *execution mechanism*, not design, so it captures the SPEC 1–3 reference work in one artifact instead of being re-edited as each change landed. No new review behavior — the script encodes the references as code, inventing nothing.
  - **R0 — `workflow/team-review.workflow.mjs` is now a committed, named, parameterized Workflow script.** The 3.8.1 "blackbox" thesis (no named/bundled script; the orchestrator re-authors the workflow from prose every run) failed at its own goal — every run still authored and wrote a 36–47 KB script to disk, then launched it via `scriptPath` — and that re-derivation dropped real safety logic (the `anyFindings` gate, `BUDGET_FLOOR`, `ARB_CAP` were lost against the older `.mjs`). The skill now reads its manifest from `args` and **launches** the committed script rather than composing one. The script encodes the full base wave shape, the Wave-1 zero-finding gate, every cap as a `const` — including the re-instated `BUDGET_FLOOR` (= 60000) and `ARB_CAP` (= 15) whose numeric seeds previously lived only in the stale `.mjs` and are now also recorded in `workflow-design.md` §Constants — the 6 adaptation points, `RESPAWN_MAX` (= 2) re-spawn + the adversary coverage-gate flag (C4), per-`review_unit`/`scoped_review` Wave-0 rule scoping and the finding-referenced reasoning-wave subsets (C2/C3), the raised static `T = 450` track threshold (R2), explicit per-spawn `model` pinning (sonnet body, opus arbiter for must-fix), and the unchanged result shape.
  - **Rule delivery — one full catalog in `args`, scoped in the script.** The Workflow runtime sandboxes the script (no filesystem or MCP access), so every rule byte an agent sees arrives in `args`. Phase 2 builds the **full** catalog once (`build_rule_package` with no arguments), `Read`s it, and passes it as `rule_packages.full`; the committed script selects each agent's `## RULES` block from that one catalog by the per-rule metadata in its rendered header — Wave-0 reviewers by `review_unit` (+ `scoped_review` for changed-method scopes), Wave-2 red team by `test_category` (the category the Wave-0 reviewers report), and the reasoning waves by the finding-referenced subset. The selection is byte-identical to a scoped `build_rule_package` call (same renderer and separator), an equivalence already CI-guarded by `plugin-tests/test-writing/selection_equivalence.bats`. This reconciles SPEC 1's "build one scoped package per track" description: the script computes the same scoping in-process, since the runtime-determined category cannot be known when the package would otherwise be pre-built.
  - **Surface — SKILL.md Phase 2/3, the digraph, and the Execution posture finalized.** Phase 2 is now "Assemble the Run Input" (build the full rule package, Pre-Run Collect the per-file fingerprint and `L > C` digest, fold both into the manifest); Phase 3 launches the committed script via `scriptPath: ${CLAUDE_SKILL_DIR}/workflow/team-review.workflow.mjs` with the manifest as `args` — no per-run authoring step remains. The digraph nodes track the rename. The Execution posture (deferred to this step by 3.8.13) drops its placeholder comment and rewrites the now-false "the review has no pre-built form to find … build it solely from the references" framing — there is a committed, named script, *launched* not composed — while keeping R1's meta-review prohibition and the input-fit check 3.8.13 added.
  - **References demoted; `input-resolution.md` manifest contract extended.** `workflow-design.md`, `reviewer-allocation.md`, `consensus-and-verdicts.md`, and `agent-guardrails.md` are marked as the human design spec / prompt-template source the committed script implements ("change the script, not just this doc"). The Phase-1 manifest now carries `test_methods` (every `public function test*` name) so the script can shard a full-class Track B file. `AGENTS.md` notes that naming a committed workflow script is naming a *tool input / shipped artifact*, not a context-delivery mechanism, so it does not violate `skill-writing.md`'s "never name the delivery mechanism" rule (which governs hooks/injection), and its Team Review diagram + modification guide point at the script as the implementation.
  - **C1 — the stale `team-review-workflow.mjs` is deleted.** The untracked repo-root research artifact (the old per-file "content-system" fan-out) was the bait an earlier run adopted instead of running the review, and a prior analysis mistook it for the migration target. With the canonical script committed it is removed; its deterministic helpers, schemas, gate, and the two re-instated caps were harvested into the committed script first, and a durable copy remains under `session-analysis-week-2026-06-24/harvest-scripts/`. No runtime file referenced it.
  - **Validation — `node --check` (wrapped as the runtime sees the body), a fixture unit test for the extractable pure helpers (`plugin-tests/test-writing/team-review-workflow-helpers.test.mjs`, 43 assertions over catalog parsing, track decision, sharding, and the consensus merge), `claude plugin validate`, the bats suite, and `validate-review-unit.sh` all pass.** The full-run behavioral metrics (0 "Prompt is too long" deaths, full red-team coverage, no full-fleet re-run, reduced units) remain the user's to confirm on a genuine team review — which should now hold reliably, since the orchestration is committed code rather than per-run prose.

## [3.8.14] - 2026-06-24

### Changed
- **Team-review migration step 3 — raise the Track-B threshold `T` (recalibrated to the post-C2 budget; revert over-aggressive method-sharding; decision stays static).** Residual revert (SPEC 3 / R2) from the Jun 21–24 multi-run analysis. Reference-text only: the per-file track decision, the allocation digraph (it references `T` symbolically), the `L > C` digest escape, and every other constant (`C` / `M` / `K_adv` / `U_file` / `G`) are unchanged; a later migration step crystallizes the same value into the committed Workflow script.
  - **R2 — `T` raised 300 → 450 (combined test+source lines).** The flat `T=300` decomposed files that comfortably fit an agent's context, inflating agent count (→ tokens and wall-clock, which is concurrency-capped at 8) with no demonstrated result gain — and the extra sharded units were the death sites in both R4 (a PersistedLayoutMutator shard) and R6 (LayoutDiagnostics shards), since more agents means more exposure to transient `529`. The seed is changed in `references/reviewer-allocation.md` (constants line) and `references/workflow-design.md` (§Constants table). The track decision **stays a static line-count threshold** — the new seed is *chosen* against the now-larger per-agent budget C2 freed up (the sharded tracks carry only scoped rules), **not** switched to a runtime "will it fit" estimate. Worked example: the 168-source + 191-test = **359-line** file the prior analysis flagged as split-with-huge-headroom now resolves to **Track A** (359 ≤ 450), as do the other named barely-over files (379 / 385 / 442). The demonstrated over-decomposition tops out at 442, so 450 captures all of it with no speculative reach into the (450, 500] band.
  - **R2.4 — Track A sanity-checked against the full catalog at the new `T`.** Raising `T` moves more files into Track A, which carries the **full** catalog (C2 scoped only the sharded tracks, since a whole-class review needs every rule type). Track A is the unchanged common path that already ran the full ~106 KB / ~49-rule catalog (~30K tokens) at `L ≤ 300` and was never a prompt-overflow death site; that catalog is the dominant, unchanged term, so raising `T` to 450 adds only ~150 combined lines (≈2K tokens) of test+source to the heaviest Track-A reviewer — negligible against the catalog and the context window. Track A also carries **no** second payload (no peer/consensus package), and every "Prompt is too long" death in the window was an agent that *did* carry that second payload (findings-A §2). So Track A stays comfortably within context at 450 and needs no scoping; the safety margin is decoupled from the 450-vs-500 choice (the line delta is negligible either way). If a future seed pushes `T` much higher the margin shrinks and Track A may then need its own scope (e.g. by `test_category`).
  - **R1 — verified, nothing to change.** Spot-checked that the team-review references hold no rule-package **file-pointer** path for spawned agents and no `get_rules` route; the inline `## RULES` block (C2-scoped) remains the only rule source for agents, and the only package `Read` is the orchestrator's own at composition time. Already corrected in step 1 (3.8.12).
  - **Tuning note — `T = 450` is a pending-validation seed.** These are frozen seed constants; this step changes only the value (and this rationale), never the mechanism, and does not run the skill. The empirical check — same changeset → fewer units/agents, no new "Prompt is too long" deaths — is the user's, on a real review run (N is small). Record before/after agent counts then; nudge `T` up toward 500 if the data supports a larger cost win, or back down if Track A tightens.
