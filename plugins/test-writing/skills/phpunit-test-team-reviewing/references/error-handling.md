...
| A `test_type` is present but its rule catalog is missing/empty | The workflow fails hard at launch (`rule_packages.{type}` is required for every test type present in the manifest). Build the missing per-type catalog in Phase 3 (`build_rule_package` with the matching group/test_type) and re-run. `rule_packages.placement` is the exception — optional reference, never required. |
