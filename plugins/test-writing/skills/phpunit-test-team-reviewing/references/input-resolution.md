...
**Hard rules:**
- Counts come from `wc -l` / exhaustive `grep`, never estimation. Enumerate **every** `public function test*` into `test_methods` — this list drives the shard count. These subagent-reported `method_count`/`test_methods` values are provisional: before the manifest freezes, the skill re-counts them deterministically from the file on disk via `workflow/verify-method-counts.sh` (SKILL.md Phase 1), replacing a mismatch with the extracted truth and logging the replacement — never merely a warning.
