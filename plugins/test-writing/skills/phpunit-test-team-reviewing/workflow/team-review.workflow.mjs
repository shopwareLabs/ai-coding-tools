...
function reviewerPrompt(unit, file, label) {
  ...
  return [
    ...
    '',
    'REMEDIATION INVARIANT: a `suggested` never changes what an existing assertion pins, and never introduces an assertion as a MEANS of satisfying a structural, layout, naming, or style constraint — a fix that must compensate that way is wrong by construction, so do not emit it. Where the finding IS itself a missing test or a missing assertion (coverage-gap rules such as MIGRATION-008, DESIGN-006, INTEGRATION-007, and missed-coverage findings), those new assertions are the explicit deliverable of that finding: name them in the finding, never smuggle them into a remediation for an unrelated rule. An assertion a fix removes rides `removed_assertions`. Re-expressing the same pinned fact in a different call (assertTrue($a === $b) becoming assertSame($b, $a), or an ISOLATION-004 literal swap) does not change what the assertion pins.',
