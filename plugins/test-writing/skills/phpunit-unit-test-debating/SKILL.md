...
Load [debate-rules.md]({baseDir}/references/debate-rules.md).
...
For each co-reviewer, send ONE message via `SendMessage(to: "{co_reviewer_name}")` covering all shared files. Use the debate message format from [output-format.md]({baseDir}/references/output-format.md):
...
Produce final stance per file using the format from [output-format.md]({baseDir}/references/output-format.md):
